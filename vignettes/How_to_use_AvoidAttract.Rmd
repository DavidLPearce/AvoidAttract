...
pdog_bobcat_ratios <- pdog_bobcat$total_summary[c(5,6)]
...
# Starting with the Black-tailed jackrabbit 
jrabbit_AAR <- AAR(data = KScams_dat, 
...
# Black-tailed Prarie Dog
pdog_AAR <- AAR(data = KScams_dat, 
                speciesA = "Black-tailed Prarie Dog", speciesB = "Coyote",
                species_col = "Common_name", datetime_col = "DateTime", 
                site_col ="Site", unitTime = "hours")
...
# Eastern cottontail rabbit
crabbit_AAR<- AAR(data = KScams_dat, 
...
```{r analysis}
# Splitting the combined ratios in to two different dataframes, one for each ratio.
T2_T1_df <- combined_ratios[, c("T2/T1", "Species")]

T4_T3_df <- combined_ratios[, c("T4/T3", "Species")]

# Perform pairwise Wilcoxon test on T2/T1
T2_T1_wilcox <- pairwise.wilcox.test(
  T2_T1_df$`T2/T1`,
  T2_T1_df$Species,
...
# Perform pairwise Wilcoxon test on T4/T3
T4_T3_wilcox <- pairwise.wilcox.test(
  T4_T3_df$`T4/T3`,
  T4_T3_df$Species,
  p.adjust.method = "bonferroni"  # Adjust p-values for multiple comparisons
)
