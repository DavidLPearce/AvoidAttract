...
#'   \describe{
#'     \item{total_summary}{A summary of the mean values for T3 across all sites that recorded an event and years (named number).}
#'     \item{event_count}{The total count of T3 events across all sites and years (named integer).}
#'     \item{event_summary}{The min1st & 3rd quartiles, median, mean, max for T3 events (dataframe).}
#'     \item{site_summary}{A summary of the mean T3 for each site across all years, if no event is recorded NA will be reported (dataframe).}
#'     \item{detailed_summary}{Detailed information on recorded T3 events, "including site, year and time" differences (dataframe).}
#'   }
#'
#' @references
...
T3 <- function(data, speciesA, speciesB, species_col, datetime_col, site_col, unitTime = "hours") {
  ...
  # Results dataframe
  detailed_summary <- data.frame(Site = character(), Year = integer(), T3 = numeric())
    ...
    for (year in years) {
      ...
      for (row in 1:(nrow(year_data) - 1)) {
        ...
        if (isTRUE(!is.na(current_species) && !is.na(next_species)) &&
          ...
          temp_result <- rbind(temp_result, data.frame(Site = site, Year = year, T3 = time_difference))
        }
      }
    }
  ...
  if (!any(!is.na(detailed_summary$T3))){
    stop("No T3 interaction events occurred. Cannot calculate a mean for this event.")
  }
  ...
  event_summary <- as.matrix(summary(detailed_summary$T3))
  colnames(event_summary) <- "T3"
